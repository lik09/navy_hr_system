...
import {
  ...
  BankOutlined, SearchOutlined, ReloadOutlined,
...
function MilitaryUnit() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('military-unit');
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        await api.put(`military-unit/${editingItem.id}`, payload);
        message.success(t('updated_succes'));
      } else {
        await api.post('military-unit', payload);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`military-unit/${id}`);
  ...
  return (
    <div>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Space align="center">
            <BankOutlined style={{ fontSize: 22, color: NAVY }} />
      ...
      <Modal
        ...
        title={
          <Space>
            <BankOutlined style={{ color: NAVY }} />
            <span>
              {editingItem
                ? `${t('edit')} ${t('military_unit')}`
                : `${t('add')} ${t('military_unit')}`}
            </span>
          </Space>
        }
        open={open}
