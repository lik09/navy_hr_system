...
import {
  ...
  ReadOutlined, SearchOutlined, ReloadOutlined,
...
function EducationLevel() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        await api.put(`education-level/${editingItem.id}`, payload);
        message.success(t('updated_succes'));
      } else {
        await api.post('education-level', payload);
  ...
  return (
    <div>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Space align="center">
            <ReadOutlined style={{ fontSize: 22, color: NAVY }} />
      ...
      <Modal
        ...
        title={
          <Space>
            <ReadOutlined style={{ color: NAVY }} />
            <span>
              {editingItem
                ? `${t('edit')} ${t('education_level')}`
                : `${t('add')} ${t('education_level')}`}
            </span>
          </Space>
        }
        open={open}
