...
function MilitarySpecialty() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('military-specialty');
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        await api.put(`military-specialty/${editingItem.id}`, payload);
        message.success(t('updated_succes'));
      } else {
        await api.post('military-specialty', payload);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`military-specialty/${id}`);
  ...
  return (
    <div>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Space align="center">
            <SafetyCertificateOutlined style={{ fontSize: 22, color: NAVY }} />
            <Title level={4} style={{ margin: 0, color: NAVY }}>
              {t('military_specialty')}
            </Title>
          </Space>
        </Col>
        <Col>
          {can('ADD_MILITARY_SPECIALTY') && (
            <Button
              type="primary"
              icon={<PlusOutlined />}
      ...
      <Modal
        ...
        title={
          <Space>
            <SafetyCertificateOutlined style={{ color: NAVY }} />
            <span>
              {editingItem
                ? `${t('edit')} ${t('military_specialty')}`
                : `${t('add')} ${t('military_specialty')}`}
            </span>
          </Space>
        }
        open={open}
...
export default MilitarySpecialty;