...
export default function PersonalInfo() {
  ...
  const toSelectOptions = (list) =>
    (list || [])
      ...
      .map(item => ({
        value: item.id,
        label: i18n.language === 'km' ? (item.name_kh || item.name) : item.name,
      }));
  ...
  const fetchOptions = async () => {
    try {
      const [ranks, positions, units, milUnits, eduLevels, specialties] = await Promise.all([
        api.get('military-rank'),
        api.get('position'),
        api.get('unit'),
        api.get('military-unit'),
        api.get('education-level'),
        api.get('military-specialty'),
      ]);
      setOptions({
        military_ranks:       ranks.data       || [],
        positions:            positions.data   || [],
        units:                units.data       || [],
        military_units:       milUnits.data    || [],
        education_levels:     eduLevels.data   || [],
        military_specialties: specialties.data || [],
      });
    } catch {
  ...
  const openEdit = (record) => {
    ...
    if (mi) {
      setMilitary({
        military_enlistment_date: mi.military_enlistment_date
          ? dayjs(mi.military_enlistment_date).format('YYYY-MM-DD') : '',
        military_rank_id:         mi.military_rank_id      || null,  // ✅ FK
        last_date_military_rank:  mi.last_date_military_rank
          ? dayjs(mi.last_date_military_rank).format('YYYY-MM-DD') : '',
        position_id:              mi.position_id           || null,  // ✅ FK
        military_specialty_id:    mi.military_specialty_id || null,  // ✅ FK
        unit_id:                  mi.unit_id               || null,  // ✅ FK
        military_unit_id:         mi.military_unit_id      || null,  // ✅ FK
        education_level_id:       mi.education_level_id    || null,  // ✅ FK
    ...
    if (fi) {
      ...
      setSpouse({
        ...
        spouse_dob:                  fi.spouse_dob
          ? dayjs(fi.spouse_dob).format('YYYY-MM-DD') : '',
        marriage_certificate_number: fi.marriage_certificate_number  || '',
        marriage_certificate_date:   fi.marriage_certificate_date
          ? dayjs(fi.marriage_certificate_date).format('YYYY-MM-DD') : '',
      });
      ...
      setChildren(fi.children
        ? fi.children.map(c => ({
            id:  c.id,
            name: c.name || '',
            dob:  c.date_of_birth ? dayjs(c.date_of_birth).format('YYYY-MM-DD') : '',
          }))
        : []);
    } else {
      setMarital(false);
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      // Step 2: Military ✅ FK ids
      if (pid) {
        const mp = {
          personal_info_id:         pid,
          military_enlistment_date: military.military_enlistment_date || null,
          military_rank_id:         military.military_rank_id         || null,
          last_date_military_rank:  military.last_date_military_rank  || null,
          position_id:              military.position_id              || null,
          military_specialty_id:    military.military_specialty_id    || null,
          unit_id:                  military.unit_id                  || null,
          military_unit_id:         military.military_unit_id         || null,
          education_level_id:       military.education_level_id       || null,
      ...
      if (errs) Object.values(errs).flat().forEach(m => message.error(m));
  ...
  const columns = [
    {
      title: t('tb_no'), key:'no', width:50, align:'center', onHeaderCell:()=>th(),
      render:(_,__,i)=>i+1,
    },
    {
      title: t('tb_id_number'), dataIndex:'id_number', key:'id_number', align:'center', onHeaderCell:()=>th(),
      render:v=><Tag color="blue" style={{borderRadius:10}}>{v||'—'}</Tag>,
    },
    {
      title: t('tb_name_kh'), dataIndex:'name_kh', key:'name_kh', onHeaderCell:()=>th({textAlign:'left'}),
      render:(v,r)=><Text strong>{v||r.name||'—'}</Text>,
    },
    { title: t('tb_name'), dataIndex:'name', key:'name', onHeaderCell:()=>th({textAlign:'left'}) },
    {
      title: t('tb_gender'), dataIndex:'gender', key:'gender', align:'center', onHeaderCell:()=>th(),
      render:v=>v==='male'?t('male'):v==='female'?t('female'):'—',
    },
    {
      title: t('tb_phone'), dataIndex:'phone_number', key:'phone_number', onHeaderCell:()=>th({textAlign:'left'}),
      render:v=>v||'—',
    },
    {
      title: t('tb_military_rank'), key:'rank', onHeaderCell:()=>th({textAlign:'left'}),
      render:(_,r) =>
        i18n.language === 'km'
          ? r.military_info?.military_rank?.name_kh || r.military_info?.military_rank?.name || '—'
          : r.military_info?.military_rank?.name || '—',
    },
    {
      title: t('tb_status'), key:'status', align:'center', onHeaderCell:()=>th(),
  ...
  if (view === 'form') return (
    <div>
      ...
      <div style={S.section}>
        ...
        <table style={S.tbl}>
          <tbody>
            {/* Row 1: Name KH + Gender + Photo */}
            <tr>
              <td style={S.lbl}>- {t('name_kh')}</td>
              <td style={S.td}>
                <TblInput value={basic.name_kh} onChange={v=>setBasic(p=>({...p,name_kh:v}))} placeholder="........................................." />
              </td>
              ...
              <td style={{...S.td,width:150}}>
                <div style={{display:'flex',gap:14}}>
                  <label style={{display:'flex',alignItems:'center',gap:3,fontSize:14}}>
                    <input type="checkbox" checked={basic.gender==='male'} onChange={()=>setBasic(p=>({...p,gender:'male'}))} />
                    {t('male')}
                  </label>
                  <label style={{display:'flex',alignItems:'center',gap:3,fontSize:14}}>
                    <input type="checkbox" checked={basic.gender==='female'} onChange={()=>setBasic(p=>({...p,gender:'female'}))} />
                    {t('female')}
                  </label>
                </div>
              </td>
              <td style={{...S.td,width:90,textAlign:'center',verticalAlign:'middle'}} rowSpan={5}>
                <Upload listType="picture-card" fileList={fileList} beforeUpload={()=>false}
                  onChange={({fileList:fl})=>setFileList(fl)} maxCount={1} accept="image/*">
                  {fileList.length===0 && (
                    <div style={{textAlign:'center'}}>
                      <CameraOutlined style={{fontSize:20,color:'#aaa'}}/>
                      <div style={{fontSize:10,color:'#aaa',marginTop:3}}>3×4</div>
                    </div>
                  )}
                </Upload>
              </td>
            ...
            <tr>
              ...
              <td style={S.td}>
                <TblInput value={basic.name} onChange={v=>setBasic(p=>({...p,name:v}))}
                  placeholder="........................................."
                  style={errors.name?{border:'1px solid red'}:{}} />
              </td>
              <td style={{...S.lbl,width:70}}>{t('id_number')}</td>
              <td style={S.td}>
                <TblInput value={basic.id_number} onChange={v=>setBasic(p=>({...p,id_number:v}))}
                  placeholder=".................."
            ...
            <tr>
              ...
              <td style={S.td} colSpan={3}>
                <TblDatePicker value={basic.date_of_birth}
                  onChange={v=>setBasic(p=>({...p,date_of_birth:v}))}
                  error={errors.date_of_birth} style={{width:140}} />
              </td>
            </tr>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={3}>
                <TblInput value={basic.military_id} onChange={v=>setBasic(p=>({...p,military_id:v}))} placeholder=".................." />
              </td>
            </tr>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={3}>
                <TblInput value={basic.civilian_id} onChange={v=>setBasic(p=>({...p,civilian_id:v}))} placeholder=".................." />
              </td>
            </tr>
            ...
            <tr>
              ...
              <LocationTd vals={birthLoc} onChange={(i,v)=>setBirthLoc(p=>{const n=[...p];n[i]=v;return n;})} colSpan={5}
            ...
            <tr>
              ...
              <LocationTd vals={currentLoc} onChange={(i,v)=>setCurrentLoc(p=>{const n=[...p];n[i]=v;return n;})} colSpan={5}
            ...
            <tr>
              ...
              <td style={S.td} colSpan={4}>
                <TblInput value={basic.phone_number} onChange={v=>setBasic(p=>({...p,phone_number:v}))} placeholder=".................." />
              </td>
            </tr>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <TblDatePicker value={military.military_enlistment_date}
                  onChange={v=>setMilitary(p=>({...p,military_enlistment_date:v}))}
                  style={{width:140}} />
              </td>
            </tr>

            {/* ✅ Military Rank — Select */}
            <tr>
              <td style={S.lbl2}>- {t('military_rank')}</td>
              <td style={S.td} colSpan={5}>
                <Select
                  className="custom-select" 
                  value={military.military_rank_id}
                  onChange={v=>setMilitary(p=>({...p,military_rank_id:v}))}
                  options={toSelectOptions(options.military_ranks)}
                  placeholder={t('pls_select_military_rank')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <Select
                  className="custom-select" 
                  value={military.position_id}
                  onChange={v=>setMilitary(p=>({...p,position_id:v}))}
                  options={toSelectOptions(options.positions)}
                  placeholder={t('pls_select_position')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <Select
                  className="custom-select" 
                  value={military.unit_id}
                  onChange={v=>setMilitary(p=>({...p,unit_id:v}))}
                  options={toSelectOptions(options.units)}
                  placeholder={t('pls_select_unit')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            {/* ✅ Military Unit — Select */}
            <tr>
              <td style={S.lbl}>- {t('military_unit')}</td>
              <td style={S.td} colSpan={5}>
                <Select
                 className="custom-select" 
                  value={military.military_unit_id}
                  onChange={v=>setMilitary(p=>({...p,military_unit_id:v}))}
                  options={toSelectOptions(options.military_units)}
                  placeholder={t('pls_select_military_unit')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <Select
                  className="custom-select" 
                  value={military.education_level_id}
                  onChange={v=>setMilitary(p=>({...p,education_level_id:v}))}
                  options={toSelectOptions(options.education_levels)}
                  placeholder={t('pls_select_education_level')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            {/* ✅ Military Specialty — Select */}
            <tr>
              <td style={S.lbl}>- {t('military_specialty')}</td>
              <td style={S.td} colSpan={5}>
                <Select
                  className="custom-select" 
                  value={military.military_specialty_id}
                  onChange={v=>setMilitary(p=>({...p,military_specialty_id:v}))}
                  options={toSelectOptions(options.military_specialties)}
                  placeholder={t('pls_select_military_specialty')}
                  allowClear
                  style={{width:260}}
                />
              </td>
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <TblDatePicker value={military.last_date_military_rank}
            ...
            <tr>
              ...
              <td style={S.td} colSpan={5}>
                <TblInput value={military.last_position}
                  onChange={v=>setMilitary(p=>({...p,last_position:v}))}
                  placeholder=".................." />
              </td>
            </tr>
            ...
            {marital && (<>
              <tr>
                ...
                <td style={S.td} colSpan={5}>
                  <div style={{display:'flex'}}>
                    <TblInput value={spouse.spouse_name} onChange={v=>setSpouse(p=>({...p,spouse_name:v}))}
                    ...
                    <div style={{display:'flex',gap:16,width:'50%',borderLeft:BORDER,paddingLeft:8}}>
                      <label style={{display:'flex',alignItems:'center',gap:3,fontSize:16,cursor:'pointer'}}>
                        <input type="checkbox" checked={!spouse.spouse_type} onChange={()=>setSpouse(p=>({...p,spouse_type:false}))} />
                        {t('husband')}
                      </label>
                      <label style={{display:'flex',alignItems:'center',gap:3,fontSize:16,cursor:'pointer'}}>
                        <input type="checkbox" checked={spouse.spouse_type} onChange={()=>setSpouse(p=>({...p,spouse_type:true}))} />
                        {t('wife')}
                      </label>
                    </div>
                  </div>
              ...
              <tr>
                ...
                <td style={S.td} colSpan={5}>
                  <TblDatePicker value={spouse.spouse_dob}
                    onChange={v=>setSpouse(p=>({...p,spouse_dob:v}))} style={{width:140}} />
                </td>
              </tr>
              <tr>
                <td style={S.lbl}>- {t('birth_location')}</td>
              ...
              <tr>
                ...
                <td colSpan={5}>
                  <div style={{paddingLeft:8,fontSize:16,display:'flex',width:'100%'}}>
                    <span>លេខ ៖ </span>
                    <TblInput value={spouse.marriage_certificate_number}
                      onChange={v=>setSpouse(p=>({...p,marriage_certificate_number:v}))}
                      placeholder=".................." style={{width:'50%',paddingLeft:8}} />
                ...
                {children.map((child, idx) => (
                  <div key={idx} style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}>
                    ...
                    <div style={{width:'50%',display:'flex',alignItems:'center',gap:4}}>
                      ...
                      <TblDatePicker value={child.dob} onChange={v=>updateChild(idx,'dob',v)} style={{width:130}} />
