...
function PermissionList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await api.get('permissions', { params: { per_page: 1000 } });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        await api.put(`permissions/${editingItem.id}`, payload);
  ...
  return (
    <div>
      ...
      <Modal
        centered
        title={editingItem ? `${t('edit')} ${t('tb_permission')}` : `${t('add')} ${t('tb_permission')}`}
        open={open}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="group" label={t('lb_group')}>
            <Input />
          </Form.Item>
